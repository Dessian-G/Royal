import { dataService, getTotalPresents, getTotalAdultes, getTotalEnfants, getTotalFinances } from '../lib/dataService'
...
export default function Statistiques() {
  const { t } = useLanguage()
  const { data: services } = useCollection(async () => {
    const all = await dataService.getServices()
    return all.slice().reverse()
  })

  if (!services.length) {
    return <div className="p-4 max-w-2xl mx-auto text-center py-12"><BarChart3 className="w-16 h-16 text-indigo-200 mx-auto mb-4" /><p className="text-indigo-600">{t.statistiques.noData}</p></div>
  }
  ...
  const presenceData = services.map(s => ({
    ...
    total: getTotalPresents(s), adultes: getTotalAdultes(s), enfants: getTotalEnfants(s),
  }))

  const financeData = services.map(s => ({
    date: new Date(s.date + 'T00:00:00').toLocaleDateString(undefined, { day: '2-digit', month: 'short' }),
    offrandes: s.offrandes || 0, dimes: s.dimes || 0, autres: s.autres_dons || 0,
  }))
    ...
    <div className="p-4 max-w-3xl mx-auto space-y-6">
      ...
      <div className="bg-white rounded-2xl p-5 shadow-sm border border-cream-dark">
        ...
        <ResponsiveContainer width="100%" height={280}>
          <LineChart data={presenceData}><XAxis dataKey="date" tick={{ fontSize: 10 }} /><YAxis tick={{ fontSize: 11 }} /><Tooltip /><Legend />
      ...
      <div className="bg-white rounded-2xl p-5 shadow-sm border border-cream-dark">
        ...
        <ResponsiveContainer width="100%" height={280}>
          <BarChart data={financeData}><XAxis dataKey="date" tick={{ fontSize: 10 }} /><YAxis tick={{ fontSize: 11 }} /><Tooltip /><Legend />
          ...
          <tbody>
            {services.slice().reverse().map(s => (
              <tr key={s.id} className="border-b border-indigo-50 hover:bg-cream">
                <td className="py-2 px-2 text-indigo-950">{new Date(s.date + 'T00:00:00').toLocaleDateString(undefined, { day: '2-digit', month: 'short', year: '2-digit' })}</td>
                ...
                <td className="py-2 px-2 text-indigo-800 truncate max-w-[150px]">{s.theme_message}</td>
...
function StatCard({ label, value }: { label: string; value: string }) {
  return <div className="bg-white rounded-xl p-4 shadow-sm border border-cream-dark text-center"><div className="text-lg font-bold text-gold">{value}</div><div className="text-xs text-indigo-600 mt-1">{label}</div></div>
}
