...
import { db, getTotalPresents, getTotalAdultes, getTotalEnfants } from '../db/database'
        ...
        <>
          ...
          <div className="bg-white rounded-2xl p-5 shadow-sm border border-cream-dark">
            ...
            <div className="text-sm text-indigo-700 mb-2 capitalize">
              {new Date(lastService.date + 'T00:00:00').toLocaleDateString(
            ...
            <div className="text-sm text-indigo-800 space-y-1">
              {lastService.dirigeant && (
                <p><span className="font-medium">{t.dashboard.dirigeant} :</span> {lastService.dirigeant}</p>
              )}
              {lastService.predicateur && (
                <p><span className="font-medium">{t.dashboard.predicateur} :</span> {lastService.predicateur}</p>
              )}
              {lastService.themeMessage && (
                <p><span className="font-medium">{t.dashboard.theme} :</span> {lastService.themeMessage}</p>
              )}
            </div>
          </div>
...
function QuickLink({ to, icon: Icon, label, color }: { to: string; icon: typeof PenLine; label: string; color: string }) {
  return (
    <Link to={to}
      className={`flex items-center gap-3 ${color} text-white rounded-xl p-4 hover:opacity-90 transition-opacity no-underline`}>
