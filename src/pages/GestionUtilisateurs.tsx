...
import { db, type UserRole, createPasswordHash } from '../db/database'
...
export default function GestionUtilisateurs() {
  ...
  async function handleCreate() {
    ...
    const existing = await db.users.where('username').equals(username.trim().toLowerCase()).first()
    if (existing) {
      showToast(labels.usernameExists, 'error')
      return
    }
  ...
  async function handleDelete(id: number) {
    if (id === currentUser?.id) {
      showToast(labels.cantDeleteSelf, 'error')
      return
    }
    if (!confirm(labels.deleteConfirm)) return
    await db.users.delete(id)
    showToast(labels.userDeleted)
  }

  async function handleResetPassword() {
    if (!resetId || !newPassword) return
    const hash = await createPasswordHash(newPassword)
    await db.users.update(resetId, { passwordHash: hash })
    ...
    <div className="p-4 max-w-2xl mx-auto space-y-6">
      ...
      {showForm && (
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-cream-dark space-y-4">
          <div>
            <label className="block text-sm font-medium text-indigo-800 mb-1">{labels.nom}</label>
          ...
          <div>
            <label className="block text-sm font-medium text-indigo-800 mb-1">{labels.role}</label>
          ...
          <div className="flex gap-3">
            <button onClick={handleCreate}
              disabled={!username.trim() || !password || !nom.trim()}
      ...
      {resetId && (
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-gold/30 space-y-4">
          ...
          <input type="password" value={newPassword} onChange={e => setNewPassword(e.target.value)}
            placeholder={labels.newPassword} autoComplete="new-password"
            className="w-full px-3 py-2.5 rounded-xl border border-indigo-200 focus:border-gold focus:ring-1 focus:ring-gold focus:outline-none" />
          <div className="flex gap-3">
            <button onClick={handleResetPassword} disabled={!newPassword}
              className="flex-1 py-3 bg-gold text-indigo-950 rounded-xl font-semibold hover:bg-gold-light disabled:opacity-40 transition-colors cursor-pointer">
              {labels.resetPassword}
            </button>
          ...
          <div key={u.id} className="bg-white rounded-xl p-4 shadow-sm border border-cream-dark flex items-center justify-between gap-3">
            <div className="flex items-center gap-3 min-w-0">
              <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${
                u.role === 'admin' ? 'bg-gold/20 text-gold' : 'bg-indigo-100 text-indigo-600'
              }`}>
            ...
            <div className="flex gap-1 shrink-0">
              <button onClick={() => { setResetId(u.id!); setNewPassword('') }}
                className="p-2 bg-gold/10 rounded-lg hover:bg-gold/20 active:bg-gold/30 transition-colors cursor-pointer"
